...
example {x y : ℝ} (h1 : y ≤ x + 5) (h2 : x ≤ -2) : x + y < 2 := calc
  x + y ≤ x + (x + 5) := by rel[h1]
  _ = 2 * x + 5 := by ring
  _ ≤ 2 * -2 + 5 := by rel[h2]
  _ < 2 := by numbers
...
-- Example 1.4.6
-- Exercise: type out the whole proof printed in the text as a Lean proof.
example {n : ℤ} (hn : n ≥ 5) : n ^ 2 > 2 * n + 11 := calc
  n^2 = n * n := by ring
  _ ≥ 5*n := by rel[hn]
  _ = 2*n + 3*n := by ring
  _ ≥ 2*n + 3*5 :=  by rel[hn]
...
example {x y : ℤ} (h1 : x + 3 ≥ 2 * y) (h2 : 1 ≤ y) : x ≥ -1 := calc
  x = (x+3) -3 := by ring
  _ ≥ 2*y - 3 := by rel[h1]
  _ ≥ 2 * 1 - 3 := by rel[h2]
  _ = -1 := by ring

example {a b : ℚ} (h1 : 3 ≤ a) (h2 : a + 2 * b ≥ 4) : a + b ≥ 3 := calc
  a + b  = 1/2 * (a+2*b + a) := by ring
  _ ≥  1/2 * (4 + 3) := by rel[h1,h2]
  _ ≥ 3 := by numbers
...
example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 := calc
...
example {n : ℤ} (h1 : n ≥ 5) : n ^ 2 - 2 * n + 3 > 14 := by 
  have h2 : n-1 ≥ 0 := calc
    ...
    _ ≥ 0 := by numbers
  calc
    n ^ 2 - 2 * n + 3 = (n-1) * (n-1) + 2 := by ring
    _ ≥ (5-1) * (n - 1) + 2 := by rel[h1]
    _ ≥ (5-1) * (5 - 1) + 2 := by rel[h1]
    _ > 14 := by numbers

example {x : ℚ} : x ^ 2 - 2 * x ≥ -1 := calc
  x^2 - 2*x = (x-1)^2 + -1 := by ring
  _ ≥ -1 := by extra

example (a b : ℝ) : a ^ 2 + b ^ 2 ≥ 2 * a * b := calc
